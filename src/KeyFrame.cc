...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    NL(F.NL), mvKeyLines(F.mvKeylinesUn),mvDepthLineStart(F.mvDepthLineStart),
    mvDepthLineEnd(F.mvDepthLineEnd), mvKeyLineFunctions(F.mvKeyLineFunctions), mLineDescriptors(F.mLdesc),
    mvpMapLines(F.mvpMapLines),mvuRightLineStart(F.mvuRightLineStart),mvuRightLineEnd(F.mvuRightLineEnd)
{
    mnId=nNextId++;
...
void KeyFrame::UpdateConnections()
{
    ...
    vector<MapLine*> vpML;
    {
        unique_lock<mutex> lockMLs(mMutexFeatures);
        vpML = mvpMapLines;
    }
    // 线的问题，
    // 我乱蒙的不知道对不对这样添加
...
{
// 
    cv::Mat Rwcobtain=Twc.rowRange(0,3).colRange(0,3);
    cv::Mat Owobtain=(Twc.rowRange(0,3).col(3));
     const float zs = mvDepthLineStart[i];
    const float  ze = mvDepthLineEnd[i];
    Vector6d Lines3D;
    if(zs>0&&ze>0)
    {
        const float us = mvKeyLines[i].startPointX;
        const float vs = mvKeyLines[i].startPointY;

        const float ue = mvKeyLines[i].endPointX;
        const float ve= mvKeyLines[i].endPointY;
        // drawKeylines();
		//计算在当前相机坐标系下的坐标
        ...
        cv::Mat A= Rwcobtain*Ac+Owobtain;
        cv::Mat Bc = (Mat_<float>(3,1) <<xe,ye,ze);
        cv::Mat B= Rwcobtain*Bc+Owobtain;

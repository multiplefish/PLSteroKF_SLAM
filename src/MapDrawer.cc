...
//关于gl相关的函数，可直接google, 并加上msdn关键词
void MapDrawer::DrawKeyFramesLine(const bool bLine)
{
   glLineWidth(5);
    glBegin ( GL_LINES );
    glColor3f(0.0,1.0,0.0); //紫色
    {
        const vector<KeyFrame*> vpKFs = mpMap->GetAllKeyFrames();
        for(size_t i=0; i<vpKFs.size(); i++)
        {
            if(i>1)
            {
               ...
               //遍历每一个关键帧，得到它在世界坐标系下的相机坐标
            cv::Mat Ow1 = vpKFs[i-1]->GetCameraCenter();
                           //遍历每一个关键帧，得到它在世界坐标系下的相机坐标
            cv::Mat OwNext = vpKFs[i]->GetCameraCenter();
            glVertex3f(Ow1.at<float>(0),Ow1.at<float>(1),Ow1.at<float>(2));
            glVertex3f(OwNext.at<float>(0),OwNext.at<float>(1),OwNext.at<float>(2));
            }
        }
    }
    glEnd();

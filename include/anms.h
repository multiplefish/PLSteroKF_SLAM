...
vector<cv::KeyPoint> sdc(vector<cv::KeyPoint> keyPoints, int numRetPoints,
                         float tolerance, int cols, int rows,bool _th) {
  int low=1; int high=0;
  if(true)
  {
  int exp1 = rows + cols + 2 * numRetPoints; 
    //4W1+4m+4H1m+H1^2+W1^2+2W1H1+4W1H1m
    long long exp2 =
    ...
    // sqrt(4W1+4m+4H1m+H1^2+W1^2+2W1H1+4W1H1m)
    ...
    // 解大于0的数求ah的最大值
    high =(sol1 > sol2)? sol1:sol2; // binary search range initialization with positive solution

    int exp5=floor(sqrt((double)keyPoints.size() / numRetPoints));
  ...
  while (!complete) {
    ...
    double c = radius / (2); // initializing Grid
    if(c==0) return keyPoints; //
